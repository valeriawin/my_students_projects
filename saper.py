...
GRID_SIZE = 8 #  Размер поля
SQUARE_SIZE = 50 # Размер клетки
...
def check_mines(neighbors): # Функция, возвращающая количество мин вокруг neighbors  
    return len(mines.intersection(neighbors))

def generate_neighbors(square): # Возвращает клетки соседствующие с square
    ...
    # Клетка в левом ряду     
        ...
        data = {square + 1, square - 1, square + GRID_SIZE, square + GRID_SIZE - 1, square + GRID_SIZE + 1}     # Клетка в правом ряду     
    elif square > GRID_SIZE ** 2 - GRID_SIZE:
        data = {square + 1, square - 1, square - GRID_SIZE, square - GRID_SIZE - 1, square - GRID_SIZE + 1}     # Клетка в нижнем ряду     
    elif square % GRID_SIZE == 0:
        data = {square + GRID_SIZE, square - GRID_SIZE, square - 1, square + GRID_SIZE - 1, square - GRID_SIZE - 1}     # Клетка в верхнем ряду     
    elif square % GRID_SIZE == 1:
        data = {square + GRID_SIZE, square - GRID_SIZE, square + 1, square + GRID_SIZE + 1, square - GRID_SIZE + 1}     # Любая другая клетка     
    else:         
...
def clearance(ids): # Итеративная функция очистки поля
    ...
    if around == 0:
        ...
        neigh_list = list(ids_neigh)         # Пока в списке соседей есть клетки         
        while len(neigh_list) > 0:
            ...
            if item_around > 0:
                ...
                if item not in clicked:
                    ...
                    c.create_text(x1 + SQUARE_SIZE / 2, y1 + SQUARE_SIZE / 2, text=str(item_around), font="Arial{}".format(int(SQUARE_SIZE / 2)), fill='yellow')             
    ...
    # Если мины вокруг есть     
        ...
        c.create_text(x1 + SQUARE_SIZE / 2, y1 + SQUARE_SIZE / 2, text=str(around), font="Arial {}".format(int(SQUARE_SIZE / 2)), fill='yellow')
...
c = Canvas(root, width=GRID_SIZE * SQUARE_SIZE, height=GRID_SIZE * 
SQUARE_SIZE)
