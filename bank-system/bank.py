...
def credit():
    ...
    if option == "1" or option == "семья":
        ...
        summa_credita = float(input(
            "Введите сумму которую хотели бы рассчитать: "
        ))
        x = summa_credita

        srok = float(input(
            "Введите на какой срок (в месяцах) хотите получить кредит: "
        ))
    ...
    if option == "3" or option == "анкета":
        ...
        person_number = int(input(
            "Выберите номер кредита 1. 'старт' или 2. 'офицерский' : "
        ))
    ...
    while t:
        ...
        if choice == "2" or choice == "моя карта":
            ...
            if menu == "2" or menu == "перевод":
                ...
                Label(text="Срок действия карты (год): ").grid(row=2, column=2, sticky=W, padx=10, pady=10)
                Spinbox(width=7, from_=2019, to=2027).grid(row=2, column=3, padx=10)
                Label(text="Срок действия карты (месяц): ").grid(row=1, column=2, sticky=E)
                Spinbox(width=7, from_=1, to=12).grid(row=1, column=3, sticky=E, padx=10)
